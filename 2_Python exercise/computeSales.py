...
def checkFile(input_File):
    ...
    for i in range(len(startStopIndexes)-1):
        ...
        for k in range(2, len(tmp)-2, 4): ##the lines with the products
            ...
            if (abs(float(tmp[k+3])-(float(tmp[k+2])*float(tmp[k+1])))<0.0001): #cast. Previously they were strings

                totalAmount=totalAmount+float(tmp[k+3])##The product has right calculated total price. We add it to the total counter              
...
def choice3():
            ...
            for line in collected_receipts_file:
                listOfLines.append(line.split())
            data={} #defining a dictionary
        #Διαβάζομε ένα ένα τα strings ώστε να τα βάλομε σε ένα λεξικό
